...
writer=ExcelWriter(r'/home/zach/Documents/Excel/export_Report_Parse.xlsx')
...
data=pd.read_excel(r'/home/zach/Desktop/python_development.xlsx', 'Summary')
Compound_Counter=0
Append=pd.DataFrame({'Data File':[], 'Compound':[],'Concentration':[]})

def parse_compound(T3Compound,T3Compound_LOD):
    T3Compound_parse=data.loc[data['Compound']==T3Compound]
    T3Compound_report=T3Compound_parse.loc[T3Compound_parse['Concentration']>T3Compound_LOD,['Data File','Compound','Concentration']]
...
for Compounds in Compound_List:
    ...
    Compound_Counter+=1
Append.set_index('Compound', inplace=True,)
Append.sort_values(by='Data File',inplace=True)
